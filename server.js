...
const DB_PATH = path.resolve(__dirname, process.env.DB_PATH || path.join('database', 'copa_presidio.sqlite'));
const SCHEMA_PATH = path.join(__dirname, 'database', 'schema.sql');
const DEFAULT_ADMIN_ACCESS_CODE = process.env.ADMIN_ACCESS_CODE || '';
const DEFAULT_ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || '';
const DATA_ENCRYPTION_KEY = parseEncryptionKey(process.env.DATA_ENCRYPTION_KEY);
...
function validateEnvironment() {
  if (!JWT_SECRET || JWT_SECRET.length < 32) {
    throw new Error('Configure JWT_SECRET no .env com pelo menos 32 caracteres.');
  }

  if (!DEFAULT_ADMIN_ACCESS_CODE || !DEFAULT_ADMIN_PASSWORD) {
    throw new Error('Configure ADMIN_ACCESS_CODE e ADMIN_PASSWORD no .env.');
  }
}
...
function parseEncryptionKey(value) {
  ...
  if (!key) {
    throw new Error('DATA_ENCRYPTION_KEY precisa ter 32 bytes em base64 ou 64 caracteres em hex.');
  }

  return key;
}

function encryptMedia(value) {
  if (!value || !DATA_ENCRYPTION_KEY || String(value).startsWith('enc:v1:')) return value || '';
...
function decryptMedia(value) {
  if (!value || !String(value).startsWith('enc:v1:')) return value || '';
  if (!DATA_ENCRYPTION_KEY) return '';

  try {
    const [, , ivValue, tagValue, encryptedValue] = String(value).split(':');
    const decipher = crypto.createDecipheriv('aes-256-gcm', DATA_ENCRYPTION_KEY, Buffer.from(ivValue, 'base64'));
    ...
    console.error('Falha ao descriptografar mídia:', error.message);
    return '';
  }
}

function encryptNullableMedia(value) {
  return value === undefined || value === null ? null : encryptMedia(value);
}

function decryptMediaFields(row, fields = ['photo', 'logo', 'team_a_logo', 'team_b_logo']) {
  if (!row) return row;
  const safe = { ...row };
  fields.forEach((field) => {
    if (Object.prototype.hasOwnProperty.call(safe, field)) safe[field] = decryptMedia(safe[field]);
  });
  return safe;
}

function decryptMediaRows(rows, fields) {
  return rows.map((row) => decryptMediaFields(row, fields));
}
...
function removeSensitivePlayerFields(player) {
  ...
  return decryptMediaFields(safe, ['photo']);
}
...
async function calculateStandings() {
  const teams = decryptMediaRows(await all('SELECT * FROM teams ORDER BY name ASC'), ['logo']);
  ...
  try {
    ...
    const created = await run(
      `INSERT INTO players (access_number, name, nickname, photo, position, desired_shirt_number, shirt_number, password_hash)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [accessNumber, name.trim(), nickname.trim(), encryptMedia(photo), position, desired_shirt_number || null, desired_shirt_number || null, hash]
    );
...
app.get('/api/player/me', requireAuth('player'), async (req, res) => {
  const team = req.player.team_id ? decryptMediaFields(await get('SELECT * FROM teams WHERE id = ?', [req.player.team_id]), ['logo']) : null;
...
app.put('/api/player/me', requireAuth('player'), async (req, res) => {
  try {
    ...
    const encryptedPhoto = encryptNullableMedia(photo);
    await run(
      `UPDATE players SET
       name = COALESCE(?, name),
       ...
       WHERE id = ?`,
      [name, nickname, encryptedPhoto, position, desired_shirt_number, req.player.id]
    );
...
app.post('/api/teams', requireAuth('player'), async (req, res) => {
  try {
    ...
    const created = await run(
      ...
      [name.trim(), encryptMedia(logo), primary_color, description, accessCode, req.player.id]
    );
    await run('UPDATE players SET team_id = ?, role = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [created.id, 'captain', req.player.id]);
    const team = decryptMediaFields(await get('SELECT * FROM teams WHERE id = ?', [created.id]), ['logo']);
...
app.get('/api/my-team', requireAuth('player'), async (req, res) => {
  try {
    ...
    const team = decryptMediaFields(await get('SELECT * FROM teams WHERE id = ?', [req.player.team_id]), ['logo']);
    const members = decryptMediaRows(await all(
...
app.put('/api/my-team', requireAuth('player'), async (req, res) => {
  try {
    ...
    const encryptedLogo = encryptNullableMedia(logo);
    await run(
      `UPDATE teams SET
       name = COALESCE(?, name),
       logo = COALESCE(?, logo),
       ...
       updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
      [name, encryptedLogo, primary_color, description, req.team.id]
    );
    const team = decryptMediaFields(await get('SELECT * FROM teams WHERE id = ?', [req.team.id]), ['logo']);
    res.json({ team });
  } catch (error) {
...
app.get('/api/public/teams', async (req, res) => {
  try {
    const teams = decryptMediaRows(await all(
...
app.get('/api/public/teams/:id', async (req, res) => {
  try {
    const team = decryptMediaFields(await get(
      `SELECT t.*, p.name as captain_name
...
app.get('/api/public/matches', async (req, res) => {
  try {
    const matches = decryptMediaRows(await all(
...
app.get('/api/public/matches/:id', async (req, res) => {
  try {
    ...
    const rosters = decryptMediaRows(await all(
...
app.post('/api/admin/login', async (req, res) => {
  try {
    ...
    const { password_hash, access_code, ...safeAdmin } = admin;
...
app.get('/api/admin/players', requireAuth('admin'), async (req, res) => {
  try {
    const players = decryptMediaRows(await all(
...
initDatabase()
  .then(() => {
    app.listen(PORT, () => {
      ...
      console.log('Credenciais administrativas carregadas pelo ambiente.');
      console.log(DATA_ENCRYPTION_KEY ? 'Criptografia de midia ativa.' : 'Criptografia de midia inativa: configure DATA_ENCRYPTION_KEY.');
    });
  })
  .catch((error) => {
